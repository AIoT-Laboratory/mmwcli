...
采集生命周期并保存原始 ADC 数据。它没有 GUI，不依赖 MATLAB、mmWave Studio、C#/.NET、
CGo、Lua host 或 TI 的 DCA1000 主机程序，也不包含 FFT、检测、可视化等数据处理链。

## 支持范围

0.1 的硬件基线是 xWR6843 ES2、单芯片、legacy frame、16-bit complex ADC 与两路硬件
LVDS。主线设备固件是 Radar Toolbox `studio_cli`，SDK demo CLI 作为独立方言支持。
...
TI 的 `studio_cli` 名称及固件名 `mmwave_Studio_cli_xwr68xx.bin` 不代表本项目依赖
mmWave Studio。固件和 TI 文档不随本仓库分发。
...
```text
mmwcli doctor --toolbox-root TOOLBOX_ROOT
mmwcli toolbox verify --toolbox-root TOOLBOX_ROOT
mmwcli studio-cli check hardware/toolbox-xwr6843-raw.cfg
```

`toolbox verify` 只接受已记录的 Radar Toolbox 4.00.00.05 资产。也可通过
`MMWCLI_RADAR_TOOLBOX_ROOT` 指定安装根目录。

## xWR6843 + DCA1000 快速开始

1. 按 TI 板卡文档烧录 Radar Toolbox 提供的
   `tools/studio_cli/prebuilt_binaries/mmwave_Studio_cli_xwr68xx.bin`。
...
| `doctor` | 离线检查 Radar Toolbox 安装 |
| `toolbox locate\|verify` | 定位或严格校验 TI 资产 |
| `studio-cli check` | 离线预检 Radar Toolbox CFG |
