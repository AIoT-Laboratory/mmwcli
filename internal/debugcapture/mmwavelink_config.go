...
func cloneRadarCapturePlan(source radar.CapturePlan) radar.CapturePlan {
	result := source
	result.ConfigurationCommands = append([]string(nil), source.ConfigurationCommands...)
	return result
}

func (plan Plan) matchesCapturePlan(source radar.CapturePlan) bool {
	return reflect.DeepEqual(plan.source, cloneRadarCapturePlan(source))
}
...
func parseMMWaveLinkConfiguration(
	...
	for _, command := range commands {
		...
		case "flushCfg":
			counts[name]++
			if len(fields) != 1 {
				return result, fmt.Errorf("debug capture requires exact flushCfg: %s", command)
			}
		case "dfeDataOutputMode":
			counts[name]++
			if !fieldsEqual(fields, "dfeDataOutputMode", "1") {
				return result, fmt.Errorf("debug capture requires exact dfeDataOutputMode 1: %s", command)
			}
		case "channelCfg":
			counts[name]++
	...
	for _, name := range []string{
		...
		if counts[name] != 1 {
			return result, fmt.Errorf("debug capture requires exactly one %s command, got %d", name, counts[name])
		}
	}
...
func parseMMWaveLinkADCBuf(fields []string, command string) (uint8, uint8, error) {
	if !fieldsEqual(fields, "adcbufCfg", "-1", "0", "1", "1", "1") {
		return 0, 0, fmt.Errorf("debug capture requires exact adcbufCfg -1 0 1 1 1: %s", command)
	}
	return 1, 1, nil
}
