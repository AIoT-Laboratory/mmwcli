...
// openEnhancedCOMConnection opens the explicit IWR6843 Enhanced COM port.
...
func openEnhancedCOMConnectionWithBackend(
	...
	partNumber, err := gateIWR6843Part(ctx, client)
	if err != nil {
		return nil, errors.Join(fmt.Errorf("gate %s part identity on %q: %w", iwr6843Platform, portName, err), client.close())
	}
...
func negotiateEnhancedCOMBaud(
	...
	if _, err := gateIWR6843Part(ctx, coldBootClient); err != nil {
		return fail(fmt.Errorf("gate %s part identity at cold-boot baud %d: %w", iwr6843Platform, enhancedCOMColdBootBaud, err))
	}
...
func gateIWR6843Part(ctx context.Context, client *enhancedCOMClient) (uint8, error) {
	...
	if !supportsIWR6843Part(partNumber) {
		return 0, unsupportedIWR6843PartError(partNumber)
	}
	return partNumber, nil
}

func supportedXWR6843Part(partNumber uint8) bool {
	return supportsIWR6843Part(partNumber)
}
...
func (connection *enhancedCOMConnection) submitFirmware(
	...
	if !connection.verified || !supportsIWR6843Part(connection.partNumber) {
		return firmwareSubmissionReceipt{}, errors.Join(
			fmt.Errorf("Enhanced COM connection has not passed the %s SOP2 monitor and part identity gate", iwr6843Platform),
			connection.close(),
		)
	}
	if ctx == nil {
