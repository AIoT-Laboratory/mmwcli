...
func TestDeriveBoardControlSelectors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: boardControlSelectors{
				reset: d2xx.Selector{Description: "AR-DevPack-EVM-012 C"},
				gpio:  d2xx.Selector{Description: "AR-DevPack-EVM-012 D"},
			},
		},
	}
	...
	if _, err := deriveBoardControlSelectors(Selectors{}); err == nil {
...
func (fixture *sop2Fixture) backend() boardControlBackend {
	return boardControlBackend{
		open: func(selector d2xx.Selector) (bitBangDevice, error) {
			...
			event := fmt.Sprintf("open:%s:%s", name, selector.Description)
