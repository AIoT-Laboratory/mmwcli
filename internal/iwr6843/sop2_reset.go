...
func prepareSOP2Target(ctx context.Context, selectors Selectors) error {
...
func deriveBoardControlSelectors(selectors Selectors) (boardControlSelectors, error) {
	if err := selectors.validate(); err != nil {
		return boardControlSelectors{}, err
	}
	base := selectors.SPI.Description[:len(selectors.SPI.Description)-2]
	return boardControlSelectors{
		reset: d2xx.Selector{Description: base + " C"},
		gpio:  d2xx.Selector{Description: base + " D"},
	}, nil
}
