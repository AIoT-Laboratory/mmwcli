...
func TestStudioDialectBaudRateAndPlatformGate(t *testing.T) {
	if StudioCLI.Name() != "studio-cli" || StudioCLI.DefaultBaud() != 921600 {
		t.Fatalf("StudioCLI = %q/%d", StudioCLI.Name(), StudioCLI.DefaultBaud())
	}
	if !StudioCLI.RequiresPlatformVerification() {
		t.Fatal("studio_cli does not require platform verification")
	}
	if StudioCLI.DeviceFamily() != xwr68xxFamily {
		t.Fatal("studio_cli must remain bound to xwr68xx")
	}
}

func TestXWR68xxDeviceFamilyDescriptor(t *testing.T) {
	family := xwr68xxFamily
	if !family.valid() || family.Name() != "xwr68xx" ||
		family.versionPlatforms != [2]string{"xWR68xx"} ||
		family.receiverMask != 0x0f || family.transmitterMask != 0x07 ||
		family.minimumStartFrequencyGHz != 57 || family.maximumStartFrequencyGHz != 64 ||
		family.adcBufBytes != 32*1024 || family.lvdsLaneCount != 2 {
		t.Fatalf("descriptor = %+v", family)
	}
	if (DeviceFamily{}).valid() {
		t.Fatal("zero device-family descriptor is valid")
	}
}
