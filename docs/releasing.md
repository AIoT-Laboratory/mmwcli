...
Release assets use the exact Git tag in their names. Tag `0.3` embeds semantic application version
`0.3.0` and retains the public asset naming contract:

- `mmwcli-0.3-linux-amd64`
- `mmwcli-0.3-linux-arm64`
- `mmwcli-0.3-windows-amd64.exe`
- `mmwcli-0.3-windows-arm64.exe`
- `mmwcli-0.3-windows-amd64-ftd2xx.exe`
...
Before creating tag `0.3`, start from a clean commit that has passed the repository checks and
verify the embedded version and all release targets:

```sh
make check
CGO_ENABLED=0 go build -trimpath -ldflags "-X=mmwcli/internal/app.Version=0.3.0" ./cmd/mmwcli
